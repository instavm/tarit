...
//!
//! PRD §4 / §5: open `/dev/kvm`, `KVM_CREATE_VM`, set up guest memory regions,
...
impl KvmVm {
    ...
    pub fn new_with_options(
        ...
        if full_boot {
            ...
            // Order matches Firecracker: TSS_ADDR → CREATE_IRQCHIP → CREATE_PIT2.
            // Do NOT call KVM_SET_IDENTITY_MAP_ADDR or KVM_SET_CLOCK —
            // Firecracker doesn't, and they may interfere with nested virt.
            vm_fd
                .create_irq_chip()
    ...
    /// so it does not disturb the running guest (Firecracker uses this for diff
    /// snapshots). Costs some write-protect overhead while enabled.
    ...
    /// IOAPIC (matching Firecracker's `register_irq`).
    pub fn register_irqfd(&self, evt: &EventFd, gsi: u32) -> Result<()> {
        ...
        // Add GSI → IOAPIC routing entry (like Firecracker's register_irq).
