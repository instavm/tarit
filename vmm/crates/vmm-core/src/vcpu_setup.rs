...
//! x86_64 vCPU register setup for direct kernel boot (PRD §5).
//!
//! Two boot modes:
...
//!   Matches Firecracker's boot protocol exactly.
...
// ============================================================
// 64-bit full boot constants (matching Firecracker)
// ============================================================
...
pub fn write_acpi_tables_with_devices(
    ...
    // DSDT: virtio first, legacy COM1/PS2 last (Firecracker order).
    ...
    // MCFG (Firecracker always includes this in XSDT).
...
/// Matches Firecracker's `PortIODeviceManager::append_aml_bytes` and
/// `add_virtio_aml` in device_manager/mmio.rs.
fn build_dsdt(devices: &[(u64, u64, u32)]) -> Result<Vec<u8>> {
    let mut aml = Vec::new();
    // Virtio first (Firecracker mmio_devices.dsdt_data order).
...
/// COM1 + i8042 PS/2 AML — byte-identical to Firecracker's
/// `PortIODeviceManager::append_aml_bytes` (legacy.rs:87).
fn append_legacy_aml(aml: &mut Vec<u8>) -> Result<()> {
    ...
    // PS2 (i8042) — required for reboot=k; matches Firecracker legacy.rs:110.
    aml::Device::new(
...
/// Virtio-mmio device AML — matches Firecracker's `add_virtio_aml`.
...
/// Build a kvm_segment from a GDT entry + table index (like Firecracker).
...
///
/// Host CPUID passthrough (matches Firecracker before normalization) with three
...
pub(crate) fn setup_vcpu_for_bzimage_boot_full_with_template(
    ...
    // Order matches Firecracker's KvmVcpu::configure:
    // CPUID → MSRs → REGS → FPU → SREGS → LAPIC

    if full_boot {
        // Set boot MSRs (matching Firecracker's create_boot_msr_entries).
    ...
    // Set REGS (matching Firecracker's setup_regs).
    let mut regs = vcpu
        .get_regs()
    ...
    // Set FPU (matching Firecracker's setup_fpu).
    if full_boot {
    ...
    // Set SREGS (matching Firecracker's setup_sregs).
    let mut sregs = vcpu
        .get_sregs()
    ...
    // Set LAPIC LINT0=EXTINT, LVT1=NMI (matching Firecracker's set_lint).
    if full_boot {
...
mod tests {
    ...
    /// Legacy COM1 AML matches Firecracker's `PortIODeviceManager` (includes _DDN).
    #[test]
    fn com1_aml_matches_firecracker() {
...
pub fn restore_vcpu_full_state(vcpu: &VcpuFd, s: &VcpuFullState) -> Result<()> {
    ...
    // current TSC), matching Firecracker's deferred-MSR ordering.
