...
impl VmmController {
    ...
    pub fn create_live(&self, config: VmConfig) -> Result<()> {
        ...
        // (confirmed against the KVM API docs §4.24/§4.38/§4.46 and Firecracker,
        // which always creates the irqchip before advertising TSC-deadline).
        let full_boot = true;
    ...
    if &magic == b"VMSN" {
        ...
        for _ in 0..n_pages {
            ...
            total_page_bytes = total_page_bytes
                .checked_add(len)
                .ok_or_else(|| VmmError::Snapshot("diff payload length overflow".into()))?;
            if total_page_bytes > crate::config::MAX_MEMORY_BYTES {
                return Err(VmmError::Snapshot(format!(
                    "diff payload too large in {path_display}: {total_page_bytes} bytes"
                )));
            }
    ...
    for (_, pages) in diffs.iter().rev() {
        for (gpa, bytes) in pages {
            ...
            if end > mem_slice.len() {
                return Err(VmmError::Snapshot(format!(
                    "diff page out of range: 0x{start:x}..0x{end:x} > 0x{:x}",
                    mem_slice.len()
                )));
            }
