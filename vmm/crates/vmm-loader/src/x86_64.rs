...
pub fn load_and_setup_boot<P: AsRef<Path>>(
    ...
    // the LinuxBoot protocol, even when the PVH ELF note is present.
    ...
    // PVH is never used: we always boot via the LinuxBoot protocol on
    // x86_64, even when the kernel carries a PVH ELF note.
    let use_pvh = false;

    // Boot params address: 0x7000 for ELF vmlinux (the conventional
    // zero-page address), 0x10000 for bzImage (setup header location).
