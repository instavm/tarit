//! Diff (incremental) snapshot builder (PRD §9a, §9c).
//!
//! PRD §9a: "Because the dirty bitmap resets on each snapshot, you can
...
//! reconstruct the full memory image byte-for-byte (PRD §12.3:
//! "base + sequence of diffs, when applied, must reproduce a full snapshot
//! byte-for-byte").
...
/// (PRD §9a: "the dirty bitmap resets on each snapshot").
pub fn build_diff(mem_bytes: &[u8], dirty: &DirtyBitmap, state: Vec<u8>) -> DiffSnapshot {
    ...
    // Deterministic page order (sorted by GPA) — required for the PRD §12.3
...
/// reconstructed full image. PRD §12.3 equivalence check.
...
mod tests {
    ...
    fn apply_diffs_reproduces_full_image() {
        // PRD §12.3: base + diffs == a full snapshot, byte-for-byte.
