//! Live snapshot — iterative pre-copy background snapshot loop (PRD §9c).
//!
//! PRD §9c: "While the guest runs, copy memory pages out to the snapshot
...
pub struct PrecopyParams {
    ...
    /// Target final-stop downtime in microseconds (PRD §2: 1-5 ms).
...
pub enum RoundDecision {
    ...
    /// Either throttle the guest or switch to post-copy (PRD §9d).
    Diverging { round: u32 },
}
...
///
/// The stop condition (PRD §9c): the residual dirty set can be copied
...
/// (PRD §12.3: "assert the pre-copy loop converges and the final pause
/// window stays under target").
