...
//!
//! Phases: Negotiation → Destination prep → Iterative pre-copy → Brief
...
//! [`vmm_snapshot::live`] (the same pre-copy / post-copy loop as
//! the live snapshot).
...
/// destination's storage continuity (storage-migration or shared
/// backing).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn postcopy_fallback_on_divergence() {
        ...
        // Diverging → Postcopy (bounds downtime regardless of dirty rate)
