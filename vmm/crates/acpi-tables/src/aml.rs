...
const AML_PKG_LENGTH_MAX: usize = 0x0fff_ffff;
...
impl Aml for Package<'_> {
    fn append_aml_bytes(&self, bytes: &mut Vec<u8>) -> Result<(), AmlError> {
        let child_count =
            u8::try_from(self.children.len()).map_err(|_| AmlError::PkgLengthTooLarge)?;
        let mut tmp = vec![child_count];
...
fn create_pkg_length(data: &[u8], include_self: bool) -> Result<Vec<u8>, AmlError> {
    create_pkg_length_from_len(data.len(), include_self)
}

fn create_pkg_length_from_len(data_len: usize, include_self: bool) -> Result<Vec<u8>, AmlError> {
    let mut result = Vec::new();

    // PkgLength is inclusive and includes the length bytes
    let length_length = if data_len < (2usize.pow(6) - 1) {
        1
    } else if data_len < (2usize.pow(12) - 2) {
        2
    } else if data_len < (2usize.pow(20) - 3) {
        3
    } else {
        4
    };
    ...
    let length = data_len
        .checked_add(if include_self { length_length } else { 0 })
        .ok_or(AmlError::PkgLengthTooLarge)?;
    if length > AML_PKG_LENGTH_MAX {
        return Err(AmlError::PkgLengthTooLarge);
    }

    match length_length {
        1 => result.push(u8::try_from(length).map_err(|_| AmlError::PkgLengthTooLarge)?),
        2 => {
            result.push((1u8 << 6) | u8::try_from(length & 0xf).unwrap());
            result.push(u8::try_from(length >> 4).map_err(|_| AmlError::PkgLengthTooLarge)?)
        }
        3 => {
            result.push((2u8 << 6) | u8::try_from(length & 0xf).unwrap());
            result.push(u8::try_from((length >> 4) & 0xff).unwrap());
            result.push(u8::try_from((length >> 12) & 0xff).unwrap());
        }
        _ => {
            result.push((3u8 << 6) | u8::try_from(length & 0xf).unwrap());
            result.push(u8::try_from((length >> 4) & 0xff).unwrap());
            result.push(u8::try_from((length >> 12) & 0xff).unwrap());
            result.push(u8::try_from((length >> 20) & 0xff).unwrap());
        }
    }
...
impl EisaName {
    pub fn new(name: &str) -> Result<Self, AmlError> {
        ...
        if !data[0..3].iter().all(|b| b.is_ascii_uppercase())
            || !data[3..7].iter().all(|b| b.is_ascii_hexdigit())
        {
            return Err(AmlError::InvalidEisaName);
        }
        let hex = |b: u8| -> u32 {
            match b {
        ...
        for field in self.fields.iter() {
            match field {
                FieldEntry::Named(name, length) => {
                    ...
                    tmp.extend_from_slice(&create_pkg_length_from_len(*length, false)?);
                }
...
mod tests {
    ...
    #[test]
    fn test_pkg_length() {
        assert_eq!(create_pkg_length(&[0u8; 62], true).unwrap(), vec![63]);
        assert_eq!(
            create_pkg_length(&[0u8; 64], true).unwrap(),
            vec![(1 << 6) | (66 & 0xf), 66 >> 4]
        );
        assert_eq!(
            create_pkg_length(&[0u8; 4096], true).unwrap(),
            vec![
                (2 << 6) | (4099 & 0xf) as u8,
        ...
        assert!(matches!(
            create_pkg_length_from_len(AML_PKG_LENGTH_MAX, true),
            Err(AmlError::PkgLengthTooLarge)
        ));
    }
    ...
    #[test]
    fn test_eisa_name_rejects_invalid_chars() {
        assert!(matches!(
            EisaName::new("0NP0501"),
            Err(AmlError::InvalidEisaName)
        ));
        assert!(matches!(
            EisaName::new("PNP0G01"),
            Err(AmlError::InvalidEisaName)
        ));
    }

    #[test]
    fn test_address_space_rejects_length_overflow() {
        let range = AddressSpace::new_io(0u16, u16::MAX).unwrap();
        assert!(matches!(range.to_aml_bytes(), Err(AmlError::AddressRange)));
    }
    #[test]
    fn test_name_path() {
        assert_eq!(
