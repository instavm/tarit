//! OCI boot-to-login diagnostic — verify the virtio-blk activation claim.
//!
...
//!     the pre-existing claim was wrong.
//!
...
/// actual evidence is the printed summary block at the end. Failing on
/// outcome would block CI when the nested-virt limitation is the very
/// thing we're documenting.
#[test]
#[ignore]
fn oci_boot_to_login_attempt() {
    ...
    let verdict = if probe.activated && probe.notify_count > 0 {
        "**Verdict A — virtio-blk activates.** DRIVER_OK observed, queue \
         kicked. The pre-existing claim that MMIO coalescing blocks \
         ...
         **Action.** The bare-metal caveat is wrong; \
    ...
    // Persist the evidence for later analysis.
    ...
    let _ = std::fs::write("target/oci-boot-probe.md", &report);
    eprintln!("wrote target/oci-boot-probe.md ({} bytes)", report.len());
}
