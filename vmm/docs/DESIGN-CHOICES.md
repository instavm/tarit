...
uncompressed. Production microVM hypervisors take the same approach
(ELF vmlinux, PVH when available, LinuxBoot otherwise).
...
### Choice: Full 11-entry boot MSR table

**Alternatives:**
1. **Host passthrough**: let KVM use its default MSRs.
2. **Safe minimal MSRs**: set only SYSENTER + STAR + TSC.
3. **Full boot MSR table**: 11 MSRs covering syscall, FPU, and MTRR
   setup.

**Why the full table:** The kernel's FPU detection, syscall
...
The full table is the proven reference in production microVMs: it sets:
...
2. `CR0 = PE | MP | ET | NE | PG` (adds MP and NE)
...
3. **Full CPUID normalization**: normalize topology, APIC
   ID, and feature bits.
...
3. **Split format**: separate state file + memory file.
...
eager copy. Lazy restore with an eager fallback is the standard
microVM restore pattern.
...
thread is required. Most VMMs use dedicated threads for all vCPUs,
