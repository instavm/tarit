...
# Downloads the kernel source and a Firecracker base config, applies the Tarit
# overrides, and builds. Output: $OUT (default $WORKDIR/vmlinux.minimal).
#
...
FC_CONFIG_URL=${FC_CONFIG_URL:-https://raw.githubusercontent.com/firecracker-microvm/firecracker/e04e55f53a25c7cf878a21b0f635a9d22cb4f3c5/resources/guest_configs/microvm-kernel-ci-x86_64-5.10-no-acpi.config}

SRC="${WORKDIR}/linux-${KERNEL_VERSION}"
TARBALL="${WORKDIR}/linux-${KERNEL_VERSION}.tar.xz"
BASE_CONFIG="${WORKDIR}/firecracker-microvm-kernel-ci-x86_64-5.10-no-acpi.config"
...
if [ ! -f "${BASE_CONFIG}" ]; then
  curl -fL "${FC_CONFIG_URL}" -o "${BASE_CONFIG}"
fi
