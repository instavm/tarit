...
- `vmm serve --socket <uds>` starts a single-VM API server. The caller sends `create`, then controls that VM with `exec`, `attach_pty`, `snapshot`, `pause`, `suspend`, `resume`, `status`, `update_egress`, and `stop` requests.
