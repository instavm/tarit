...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let max_memory_mib = env::var("TARIT_MAX_MEMORY_MIB")
            ...
            .unwrap_or(65_536);
        let peer_secret =
            env::var("TARIT_PEER_SECRET").unwrap_or_else(|_| "dev-peer-secret".into());
        ...
        // Cluster security: the shared peer secret authenticates all cross-node
        // control; refuse the built-in dev default in multi-node mode so peers
        // cannot be spoofed.
        if database_url.is_some() && peer_secret == "dev-peer-secret" {
            bail!("TARIT_PEER_SECRET must be set to a strong value (not the dev default) when TARIT_DATABASE_URL is configured for a fleet");
        }
