...
MAX_VCPUS="${TARIT_LIFECYCLE_MAX_VCPUS:-$MAX_VMS}"
MAX_MEMORY_MIB="${TARIT_LIFECYCLE_MAX_MEMORY_MIB:-$((MAX_VMS * 256))}"
[[ "$MAX_VMS" =~ ^[1-9][0-9]*$ ]] || {
  echo "FAIL: TARIT_LIFECYCLE_MAX_VMS must be a positive integer" >&2
  exit 1
}
[[ "$MAX_VCPUS" =~ ^[1-9][0-9]*$ ]] || {
  echo "FAIL: TARIT_LIFECYCLE_MAX_VCPUS must be a positive integer" >&2
  exit 1
}
[[ "$MAX_MEMORY_MIB" =~ ^[1-9][0-9]*$ ]] || {
  echo "FAIL: TARIT_LIFECYCLE_MAX_MEMORY_MIB must be a positive integer" >&2
  exit 1
}
...
TARIT_MAX_VCPUS="$MAX_VCPUS" \
TARIT_MAX_MEMORY_MIB="$MAX_MEMORY_MIB" \
