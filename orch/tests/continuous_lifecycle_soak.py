...
class Soak:
    ...
    def fork_summary(self) -> dict[str, object]:
        ...
        return {
            ...
            "by_source_vcpus": {
                ...
                for vcpus, samples in sorted(self.fork_metric_samples_by_vcpu.items())
            },
        }
    ...
    def record_fork_metrics(
        ...
        self.fork_terminations[termination] = (
            self.fork_terminations.get(termination, 0) + 1
        )
    ...
    def write_status(self, kind: str, fields: dict[str, object]) -> None:
        ...
        payload = {
            "schema_version": 1,
            "state": "failed"
            if kind == "soak_failure"
            else ("passed" if kind == "soak_pass" else "running"),
            "latest_event": kind,
...
""".replace("PROOF_VALUE", proof)
    ...
    def fork_anchor(self, vm_id: str) -> None:
        ...
        assert (
            self.exec(child_id, "cat /root/tarit-soak-proof")
            == self.anchors[vm_id].proof
        )
        ...
        assert (
            self.exec(vm_id, "cat /root/tarit-soak-proof") == self.anchors[vm_id].proof
        )
    ...
    def snapshot_restore(self, vm_id: str) -> None:
        ...
        assert (
            self.exec(restored_id, "cat /root/tarit-soak-proof")
            == self.anchors[vm_id].proof
        )
    ...
    def hibernate_resume(self, vm_id: str) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
            futures = [
                pool.submit(self.exec, vm_id, f"echo wake-{index}")
                for index in range(4)
            ]
            assert [future.result(timeout=360) for future in futures] == [
                f"wake-{index}" for index in range(4)
            ]
    ...
    def start_epoch_hibernation(self) -> None:
        ...
        _, row = self.request("POST", "/v1/vms", {"vcpus": 1, "memory_mib": 256}, 201)
        vm_id = row["id"]
        assert row["status"] == "running", row
        self.install_workload(vm_id, proof)
        before = self.workload_state(vm_id)
    ...
    def finish_epoch_hibernation(self) -> None:
        ...
        assert hold_seconds >= self.args.epoch_hibernate_min_seconds, (
            hold_seconds,
            self.args.epoch_hibernate_min_seconds,
        )
    ...
    def long_hibernate_clock_timer(self, vm_id: str) -> None:
        ...
        assert uptime_after - uptime_before < timer_seconds, (
            uptime_before,
            uptime_after,
        )
        assert abs(realtime_after - host_realtime) <= 3, (realtime_after, host_realtime)
        assert (
            realtime_after - realtime_before >= self.args.hibernate_hold_seconds - 3
        ), (
            realtime_before,
            realtime_after,
        )
        ...
        while time.monotonic() < realtime_timer_deadline:
            ...
            raise AssertionError(
                "expired absolute-realtime timer did not fire after repair"
            )
        ...
        while time.monotonic() < timer_deadline:
            if (
                self.exec(
                    vm_id,
                    "if test -e /tmp/tarit-timer-fired; then echo fired; else echo pending; fi",
                )
                == "fired"
            ):
                break
    ...
    def sibling_fork_timers(self, vm_id: str) -> None:
        ...
        for child_state in child_states:
            ...
            assert (
                child_state["counter"] == "0" and child_state["cache"] == "-"
            ), child_state
        for field in ("clone", "prng", "ticket", "prefix"):
        ...
        for timer_vm_id in vm_ids:
            ...
            assert fired[timer_vm_id]["realtime"] <= max(
                timer_seconds + 3,
                fork_ready.get(timer_vm_id, 0) + 3,
            ), fired
    ...
    def assert_global_invariants(self) -> None:
        ...
        for vm_id in self.anchors:
            assert vm_id in by_id and by_id[vm_id]["status"] == "running", (
                vm_id,
                by_id,
            )
        if self.sentinel:
        ...
        snapshot_dir = os.path.join(
            os.path.dirname(self.args.database), "sockets", "snapshots"
        )
    ...
    def cleanup(self) -> None:
        ...
        for vm_id in list(self.transient) + sentinel_ids + list(self.anchors):
            try:
                self.request(
                    "DELETE", f"/v1/vms/{vm_id}", expected={204, 404}, timeout=120
                )
    ...
    def run(self) -> None:
        ...
        if self.args.actions:
            ...
            actions = [
                ...
                self.mutate,
                self.fork_anchor,
                self.hibernate_resume,
                self.pause_resume,
                self.balloon,
                self.concurrent_guest_work,
                self.contended_guest_agent_exec,
            ]
        deadline = (
        ...
        while (
            ...
            else completed_steps < self.args.steps
        ):
        ...
        if self.sentinel:
            ...
            self.assert_global_invariants()
        minimum_age = min(
            time.monotonic() - anchor.created_at for anchor in self.anchors.values()
        )
        self.event(
            "soak_pass",
            seed=self.args.seed,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--status-file",
        default=os.environ.get("TARIT_LIFECYCLE_STATUS_FILE"),
    )
    parser.add_argument(
        "--case-name",
        default=os.environ.get("TARIT_LIFECYCLE_CASE_NAME"),
    )
    parser.add_argument(
        "--epoch",
        type=int,
        default=int(os.environ.get("TARIT_LIFECYCLE_EPOCH", "0")),
    )
    ...
    if any(
        ...
        parser.error(
            "seeds must contain 1 to 64 comma-separated unsigned 64-bit integers"
        )
    args.seeds = [int(value) for value in seed_values]
    if (
        not args.seeds
        or len(args.seeds) > 64
    ...
    if not args.anchor_vcpus or any(
        value < 1 or value > 8 for value in args.anchor_vcpus
    ):
    ...
    if (
        args.hibernate_hold_seconds
        and args.hibernate_hold_seconds <= args.guest_timer_seconds + 2
    ):
        parser.error(
            "hibernate hold must exceed the guest timer by at least three seconds"
        )
