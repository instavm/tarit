...
This document describes the distributed design implemented in `crates/taritd` and `crates/tarit-fleet`. Single-host mode and cluster mode are the same binary; cluster mode is selected by setting `TARIT_DATABASE_URL` and a non-default `TARIT_PEER_SECRET`.
