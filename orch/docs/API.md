...
| PTY WebSocket | `?api_key=<api key>` | `/v1/vms/{id}/pty/{pty_id}/connect`. |
...
```json
{
  ...
  "memory_mib": 256,
  "vcpus": 1,
  "kernel_path": "optional kernel override",
  "image": "optional registered image name[:tag]",
  "rootfs_path": "optional rootfs override, empty string means no rootfs",
...
`WS /v1/vms/{id}/pty/{pty_id}/connect?api_key=...` upgrades to a WebSocket. Binary messages are raw PTY bytes. Text messages are JSON controls: client-to-server `{"type":"resize","cols":N,"rows":N}` and server-to-client `{"type":"exit","exit_code":N}`.

Status codes: `200`, `201`, `204`, `400`, `401`, `404`, `409`, `500`. WebSocket auth failures close with code `4401`.
...
Internal handlers call the same `ops::*_local` functions as public handlers, so local behavior is shared. Peer-forwarded public requests include the resolved tenant/role in internal headers so the owner node can enforce tenant access before local VM operations. They intentionally do not perform owner resolution.
