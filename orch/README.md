...
Single-host mode is selected by omitting `TARIT_DATABASE_URL`. Cluster mode is selected by setting `TARIT_DATABASE_URL` and a non-default `TARIT_PEER_SECRET` on every node.
...
| WS | `/v1/vms/{id}/pty/{pty_id}/connect?api_key=...` | Bridge WebSocket PTY bytes to the owning local VMM stream. |
...
| `TARIT_PEER_SECRET` | Cluster: yes | `dev-peer-secret` | Shared secret for `X-Peer-Secret`. The default is refused when `TARIT_DATABASE_URL` is set. |
